<script lang="ts">
	...
	let imgURL: string = "";
	let hasManifesto: boolean = false;
	...
	$: pb.collection('candidates')
		...
		.then((record)=>{
			currentRecord=record;
			if (record.manifestos !== ""){
				console.log(record.manifestos)
				hasManifesto=true;
				imgURL=pb.getFileUrl(record, record.manifestos);
				console.log(imgURL)
			}
		})
...
<Content>
	{#if currentRecord !== undefined}
		<Grid padding>
			<Row>
				<Column>
					<h1>{currentRecord.first_name}, {currentRecord.last_name}</h1>                    
				</Column>
			</Row>
			<Row>
				<Column>
					{#if imgURL !== ""}
						<ImageLoader src={imgURL}></ImageLoader>
					{/if}
